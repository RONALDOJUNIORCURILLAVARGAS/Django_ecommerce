...
from .models import OrderItem, Product,Address,Payment
...
class CheckoutView(generic.FormView):
    ...
    def form_valid(self, form):
        ...
        if selected_shipping_address: #TRUE
            order.shipping_address=selected_shipping_address
        else:
            address=Address.objects.create(
                address_type= 'S',
                user=self.request.user,
                address_line_1=form.cleaned_data['shipping_addres_line_1'],
                address_line_2=form.cleaned_data['shipping_addres_line_2'],
                zip_code=form.cleaned_data['shipping_zip_code'],
                city=form.cleaned_data['shipping_city'],
            )
            order.shipping_address=address

        if selected_billing_address: #TRUE
            order.billing_address=selected_billing_address
        else:
            address=Address.objects.create(
                address_type= 'B',
                user=self.request.user,
                address_line_1=form.cleaned_data['billing_addres_line_1'],
                address_line_2=form.cleaned_data['billing_addres_line_2'],
...
class ConfirmOrderView(generic.View):
    def post(self, request, *args,**kwargs):
        ...
        payment=Payment.objects.create(
            ...
            amount= float(body["purcharse_units"][0]["amount"]["value"]),
